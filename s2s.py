...
## https://blog.csdn.net/gitchat/article/details/79081190
...
def test_general_ques():
    ...
    with tf.Session() as sess:
        ...
        output_name = 'training'+ckpt.model_checkpoint_path.split('/')[-2][-1] +'-'+ ckpt.model_checkpoint_path.split('/')[-1] + '_qa_test_' + datetime.datetime.now().strftime("%Y-%B-%d-%I-%M") + '.txt'
        output_file = open('/home/honghaier1688/workspaces/Seq2Seq_Chatbot_QA-master/test_auto_result/'+ output_name, "w")
        output_file.write('restore model file %s \n' % ckpt.model_checkpoint_path)
        with open('/home/honghaier1688/workspaces/Seq2Seq_Chatbot_QA-master/test_auto_result/general_ques.txt') as f:
            while True:
                sentence = f.readline()
                if "exit()" in sentence:
                    break
